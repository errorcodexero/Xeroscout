...
logging.debug("Inside server.py")
...
class ScoutServer(object):
    ...
    def __init__(self):
        logging.debug("Init ScoutServer")
    ...
    def team(self, n="238"):
        ...
        if lastEvent:
          try:
              # TODO Convert to mysql? What's going on here?
    ...
    def flag(self, num='', match='', flagval=0):
        ...
        # TODO What's going on here? Convert to mysql?
        conn = sql.connect(self.datapath())
        conn.row_factory = sql.Row
        cursor = conn.cursor()
    ...
    def submit(self, auth='', data='', pitData='', event='', team='', comment=''):
        ...
        datapath = 'data_' + event + '.db'
        self.database_exists(event)
        # Connect to mysql database. Connection constants are set at the top of this file.
        conn = mysql.connector.connect(user=db_user, password=db_password,
                                       host=db_host,
                                       database=db_database)
        cursor = conn.cursor()
        # Set default schema to event
        cursor.execute("use data_" + event)

        if data:
            d = literal_eval(data)
            logging.debug("Match data detected")
            # If replay is marked, replace previous data
            if d['Replay']:   # replay
                logging.debug("Replay Detected")
                query = "DELETE from scout WHERE Team = '%s' AND `Match` = '%s'" % (str(d['Team']), str(d['Match']))
                logging.debug(query)
                cursor.execute(query)

            # Insert data into database
            logging.debug("Prepare scouting insert")
            scout_insert = 'INSERT INTO scout ('
            for key in d:
                scout_insert += '`' + key + "`, "
            ...
            logging.debug("Pit data detected")
            query = "DELETE from pitScout WHERE Team='%s'" % str(d['Team'])
            logging.debug(query)
            cursor.execute(query)

            # Build the pitScout insert statement
            logging.debug("Prepare Pit scouting insert")
            scout_insert = 'INSERT INTO pitscout ('
            for key in d:
                scout_insert += "`" + key + "`, "
            scout_insert = scout_insert[:-2]  # strip the trailing comma and space
            scout_insert += ") VALUES (" \
    ...
    def submit_old(self, auth='', data='', pitData='', event='', team='', comment=''):
        ...
        if auth == serverinfo.AUTH:
            if data:
                ...
                # Insert data into database
                scout_insert = 'INSERT INTO scout (key, '
                for key in d:
                    scout_insert += key + ", "
                scout_insert = scout_insert[:-2]  # strip the trailing comma and space
                scout_insert += ') VALUES (NULL,' + ','.join([str(a) for a in d.values()]) + ')'
                print(scout_insert)
                cursor.execute(scout_insert)
                conn.commit()
                conn.close()

                # Recalculate stats for new data
    ...
    def database_exists(self, event):
        """
        Creates the event schema in MySQL if it does not exist
        :param event:
        :return:
        """
        ...
        logging.debug("Checking existence of event schema")
        if (schema,) not in rows:
            # Create schema
            query = "create schema " + db_database
            cursor.execute(query)
            query = "use " + schema
            cursor.execute(query)
            ...
            # FOR TESTING PURPOSES ONLY. IS THIS MESSING UP THE CODE?
    ...
    def database_exists_old(self, event):
        datapath = 'data_' + event + '.db'
...
if len(sys.argv) > 1:
    if sys.argv[1] == "-local":
        ...
        logging.debug("Start " + str(datetime.datetime.now()))
        localInstance = True
        logging.debug("Calling ScoutServer Process")
        try:
